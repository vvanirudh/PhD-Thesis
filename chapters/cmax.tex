...
\paragraph{A Closer Look at the Assumption}
\label{sec:closer-look-at-1}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.5\linewidth]{figures/cmax/example.pdf}
  \caption{A small icy gridworld where Assumption~\ref{assumption:core} is
    satisfied but Assumption~\ref{assumption:strong} is not satisfied.}
  \label{fig:example}
\end{figure}
...
small icy gridworld shown in Figure~\ref{fig:example}
...
We can also interpret this assumption as a variant of the \textit{optimistic
  model} assumption that is presented in Section~\ref{sec:guarantees} and is
introduced in~\cite{DBLP:conf/aaai/Jiang18}. Specifically, an optimistic model
is an approximate model $\hat{M}$ whose optimal cost-to-go under its dynamics
$\hat{f}$ at every state $s \in \statespace$ underestimates teh optimal
cost-to-go under true dynamics $f$. A popular example of the optimistic model
assumption is the free space assumption~\cite{DBLP:journals/trob/Zelinsky92}
that is typically used in tasks
involving navigation in unknown environments, where we assume that any unknown
region in the map is free of obstacles. Assumption~\ref{assumption:core} can be
interpreted as requiring that the model $\hat{M}_{t}$ remains optimistic in
Algorithm~\ref{alg:small-state-spaces} until the robot reaches the goal.
Unfortunately, this interpretation does not make the assumption easier to verify
prior to execution as it still depends on the execution trace of \cmax{}.
...
The most important shortcomings of \textsc{Cmax}
are
